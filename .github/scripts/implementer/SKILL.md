...
2. Within that milestone, find the **first** `- [ ]` / `* [ ]` step that is not preceded by an unresolved `**Blocked by:**` annotation. That step is the candidate. **If the candidate begins with the human marker → this run is Step 3 Case C. STOP. Do NOT continue scanning for a later non-human step** — the marker is a hard blocker for the whole milestone, not a skip instruction (per `templates/planning/INSTRUCTIONS.md` "Semantics": downstream steps almost always depend on the human action; the 2026-06-04 second incident happened exactly this way — the agent skipped `_(human)_` M24.9, walked into the dependent HIGH-tier M24.10, and burned max-turns). **Human-marker detection MUST be emphasis-agnostic:** the marker is `(human)` wrapped in any markdown emphasis — `*(human)*`, `_(human)_`, `**(human)**`, or `__(human)__` — immediately after the checkbox. All variants render identically and mean the same thing; never detect by literal `*(human)*` string match alone (that caused the 2026-06-04 first incident). If the candidate is not human-marked, it is the work item. Its text is everything between the checkbox and the next checkbox / heading / horizontal rule; acceptance criteria are the indented bullets that follow.
3. If the milestone has **no** candidate step (all done or all blocked), log `::notice::implementer: directed milestone <target_ref> has no unblocked step; nothing to do` and exit cleanly with tick-outcome `guard-tripped`. (The picker only offers milestones with ≥1 unblocked step and the start route 422s stale targets, but a step can be consumed between enumeration and dispatch — handle it defensively, never by diverting to another target.)
